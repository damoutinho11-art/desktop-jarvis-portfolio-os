"""Readable report for FTAW identity guard submission dry-run pack."""
...
def build_ftaw_identity_guard_submission_dry_run_pack_report(
    ...
    pack = build_ftaw_identity_guard_submission_dry_run_pack_from_files(
    ...
    lines = [
        "J.A.R.V.I.S. FTAW Identity Guard Submission Dry-Run Pack Report",
        "Automated structure. Manual trust.",
        f"target asset: {pack.target_asset}",
        f"dry-run status: {pack.dry_run_status}",
        f"upstream v4.33 status: {pack.upstream_v4_33_status}",
        f"accepted decision count: {pack.accepted_decision_count}",
        f"required public decision count: {pack.required_public_decision_count}",
        f"missing decision count: {pack.missing_decision_count}",
        f"dry-run packet item count: {pack.dry_run_packet_item_count}",
        f"manual/private outstanding count: {pack.manual_private_outstanding_count}",
        "packet table:",
        "evidence_type | source_reference_id | reviewer_decision | included_in_dry_run_packet | identity_guard_executed | identity_guard_pass_created | queue_eligibility_created | evidence_verified | approved_asset | buy_signal",
    ]
    if pack.packet_items:
        for item in pack.packet_items:
            lines.append(
                f"{item.evidence_type} | {item.source_reference_id} | {item.reviewer_decision} | "
                f"{str(item.included_in_dry_run_packet).lower()} | {str(item.identity_guard_executed).lower()} | "
                f"{str(item.identity_guard_pass_created).lower()} | {str(item.queue_eligibility_created).lower()} | "
                f"{str(item.evidence_verified).lower()} | {str(item.approved_asset).lower()} | {str(item.buy_signal).lower()}"
            )
    else:
        lines.append("none")
    ...
    lines.extend(
        [
            ...
            "identity guard submission dry-run is not identity confirmation.",
            "identity guard submission dry-run is not evidence verification.",
            "identity guard submission dry-run is not queue eligibility.",
            "identity guard submission dry-run is not asset approval.",
            "identity guard submission dry-run is not registry mutation.",
            "identity guard submission dry-run is not allocation advice.",
            "identity guard submission dry-run is not a buy/sell request.",
            "identity guard submission dry-run is not trade execution.",
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Build FTAW identity guard submission dry-run report.")
