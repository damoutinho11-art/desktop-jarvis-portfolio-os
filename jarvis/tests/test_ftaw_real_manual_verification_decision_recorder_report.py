...
from jarvis.ftaw_real_manual_verification_decision_recorder_report import (
...
def _report(
    ...
    return build_ftaw_real_manual_verification_decision_recorder_report(
        SOURCE_REGISTRY,
        None,
        URL_FETCH_CONFIG,
        INTAKE_CONFIG,
...
class FTAWRealManualVerificationDecisionRecorderReportTests(unittest.TestCase):
    def test_default_report_prints_blocked_status_decision_table_and_safety_lines(self) -> None:
        report = _report()

        self.assertIn("decision recorder status: BLOCKED_NO_VERIFICATION_QUEUE_DRY_RUN", report)
    ...
    def test_partial_report_prints_partial_status_and_blockers(self) -> None:
        ...
        self.assertIn("decision recorder status: PARTIAL_MANUAL_VERIFICATION_DECISIONS_RECORDED", report)
        self.assertIn("rejected/needs_correction count:", report)
        self.assertIn("blocked reasons:", report)
    ...
    def test_complete_report_prints_recorded_status_warnings_and_safety_lines(self) -> None:
        ...
        self.assertIn("decision recorder status: MANUAL_VERIFICATION_DECISIONS_RECORDED_FOR_VERIFIED_EVIDENCE_PREVIEW_REVIEW", report)
        self.assertIn("decision table:", report)
        for line in (
    ...
    def test_cli_report_runs_without_error_and_prints_decision_table_and_safety_lines(self) -> None:
        result = subprocess.run(
            [
                sys.executable,
                "-m",
                "jarvis.ftaw_real_manual_verification_decision_recorder_report",
                SOURCE_REGISTRY,
                "None",
                URL_FETCH_CONFIG,
                INTAKE_CONFIG,
        ...
        self.assertIn("decision recorder status: MANUAL_VERIFICATION_DECISIONS_RECORDED_FOR_VERIFIED_EVIDENCE_PREVIEW_REVIEW", result.stdout)
        self.assertIn("decision table:", result.stdout)
