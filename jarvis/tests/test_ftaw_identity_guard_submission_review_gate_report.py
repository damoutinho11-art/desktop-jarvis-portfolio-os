...
from jarvis.ftaw_identity_guard_submission_review_gate_report import (
    build_ftaw_identity_guard_submission_review_gate_report,
)
...
def _report(
    ...
    return build_ftaw_identity_guard_submission_review_gate_report(
        SOURCE_REGISTRY,
        None,
        URL_FETCH_CONFIG,
        INTAKE_CONFIG,
...
class FTAWIdentityGuardSubmissionReviewGateReportTests(unittest.TestCase):
    def test_default_report_prints_blocked_status_readiness_checks_and_safety_lines(self) -> None:
        report = _report()

        self.assertIn("gate status: BLOCKED_NO_IDENTITY_GUARD_DRY_RUN_PACKET", report)
        self.assertIn("readiness checks table:", report)
    ...
    def test_partial_report_prints_partial_status_and_blockers(self) -> None:
        ...
        self.assertIn("gate status: PARTIAL_IDENTITY_GUARD_SUBMISSION_REVIEW_READY", report)
        self.assertIn("missing packet item count: 4", report)
        self.assertIn("blocked reasons:", report)

    def test_complete_report_prints_ready_status_warnings_and_all_safety_lines(self) -> None:
        report = _report(
            COMPLETE_QUEUE_CONFIG,
            COMPLETE_DECISION_CONFIG,
            COMPLETE_PREVIEW_CONFIG,
        ...
        self.assertIn("gate status: READY_FOR_EXPLICIT_MANUAL_IDENTITY_GUARD_SUBMISSION_COMMAND", report)
        self.assertIn("readiness checks table:", report)
        self.assertIn("identity guard submission review gate is not identity confirmation.", report)
        self.assertIn("identity guard submission review gate is not evidence verification.", report)
        self.assertIn("identity guard submission review gate is not queue eligibility.", report)
        self.assertIn("identity guard submission review gate is not asset approval.", report)
        self.assertIn("identity guard submission review gate is not registry mutation.", report)
        self.assertIn("identity guard submission review gate is not allocation advice.", report)
        self.assertIn("identity guard submission review gate is not a buy/sell request.", report)
        self.assertIn("identity guard submission review gate is not trade execution.", report)
        for line in (
    ...
    def test_cli_report_runs_without_error_and_prints_readiness_checks_and_safety_lines(self) -> None:
        result = subprocess.run(
            [
                sys.executable,
                "-m",
                "jarvis.ftaw_identity_guard_submission_review_gate_report",
                SOURCE_REGISTRY,
                "None",
                URL_FETCH_CONFIG,
                INTAKE_CONFIG,
        ...
        self.assertIn("gate status: READY_FOR_EXPLICIT_MANUAL_IDENTITY_GUARD_SUBMISSION_COMMAND", result.stdout)
        self.assertIn("readiness checks table:", result.stdout)
