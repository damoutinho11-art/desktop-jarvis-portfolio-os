...
from jarvis.ftaw_registry_update_apply_gate_report import build_ftaw_registry_update_apply_gate_report
...
def _report(
    ...
    return build_ftaw_registry_update_apply_gate_report(
        SOURCE_REGISTRY,
        None,
        URL_FETCH_CONFIG,
        INTAKE_CONFIG,
...
class FTAWRegistryUpdateApplyGateReportTests(unittest.TestCase):
    def test_default_report_blocks_and_prints_safety_lines(self) -> None:
        report = _report()

        self.assertIn("J.A.R.V.I.S. FTAW Registry Update Apply Gate Report", report)
        self.assertIn("apply gate status: BLOCKED", report)
        self.assertIn("explicit_manual_apply_required: true", report)
        self.assertIn("apply_executed: false", report)
        self.assertIn("registry_file_written: false", report)
        self.assertIn("apply gate readiness is not registry mutation.", report)
        self.assertIn("apply gate readiness is not asset approval.", report)
        self.assertIn("apply gate readiness is not verified evidence promotion.", report)
        self.assertIn("apply gate readiness is not allocation advice.", report)
        self.assertIn("apply gate readiness is not a buy/sell request.", report)
        self.assertIn("apply gate readiness is not trade execution.", report)
    ...
    def test_synthetic_complete_report_reaches_ready_for_explicit_manual_apply(self) -> None:
        report = _report(
        ...
        self.assertIn("apply gate status: READY_FOR_EXPLICIT_MANUAL_REGISTRY_APPLY", report)
        self.assertIn("registry_update_mode: dry_run", report)
        self.assertIn("registry_mutation: false", report)
    ...
    def test_cli_report_runs_without_error(self) -> None:
        result = subprocess.run(
            [
                sys.executable,
                "-m",
                "jarvis.ftaw_registry_update_apply_gate_report",
                SOURCE_REGISTRY,
                "None",
                URL_FETCH_CONFIG,
                INTAKE_CONFIG,
        ...
        self.assertIn("apply gate status: READY_FOR_EXPLICIT_MANUAL_REGISTRY_APPLY", result.stdout)
