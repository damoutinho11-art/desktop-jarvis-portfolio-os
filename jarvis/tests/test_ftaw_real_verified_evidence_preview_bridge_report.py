...
from jarvis.ftaw_real_verified_evidence_preview_bridge_report import (
...
def _report(
    ...
    return build_ftaw_real_verified_evidence_preview_bridge_report(
        SOURCE_REGISTRY,
        None,
        URL_FETCH_CONFIG,
        INTAKE_CONFIG,
...
class FTAWRealVerifiedEvidencePreviewBridgeReportTests(unittest.TestCase):
    def test_default_report_prints_blocked_status_preview_table_and_safety_lines(self) -> None:
        report = _report()

        self.assertIn("preview bridge status: BLOCKED_NO_MANUAL_VERIFICATION_DECISIONS", report)
        self.assertIn("preview table:", report)
    ...
    def test_partial_report_prints_partial_status_and_blockers(self) -> None:
        ...
        self.assertIn("preview bridge status: PARTIAL_VERIFIED_EVIDENCE_PREVIEW_READY", report)
        self.assertIn("preview record count: 1", report)
        self.assertIn("blocked reasons:", report)
    ...
    def test_complete_report_prints_ready_status_warnings_and_safety_lines(self) -> None:
        ...
        self.assertIn("preview bridge status: VERIFIED_EVIDENCE_PREVIEW_READY_FOR_PROMOTION_DRY_RUN_REVIEW", report)
        self.assertIn("preview record count: 5", report)
        for line in (
    ...
    def test_cli_report_runs_without_error_and_prints_preview_table_and_safety_lines(self) -> None:
        result = subprocess.run(
            [
                sys.executable,
                "-m",
                "jarvis.ftaw_real_verified_evidence_preview_bridge_report",
                SOURCE_REGISTRY,
                "None",
                URL_FETCH_CONFIG,
                INTAKE_CONFIG,
        ...
        self.assertIn("preview bridge status: VERIFIED_EVIDENCE_PREVIEW_READY_FOR_PROMOTION_DRY_RUN_REVIEW", result.stdout)
        self.assertIn("preview table:", result.stdout)
