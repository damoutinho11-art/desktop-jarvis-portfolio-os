...
from jarvis.ftaw_human_approval_review_decision_recorder_report import build_ftaw_human_approval_review_decision_recorder_report
...
def _report(
    ...
    return build_ftaw_human_approval_review_decision_recorder_report(
        SOURCE_REGISTRY,
        None,
        URL_FETCH_CONFIG,
        INTAKE_CONFIG,
...
class FTAWHumanApprovalReviewDecisionRecorderReportTests(unittest.TestCase):
    def test_default_report_blocks_and_prints_safety_lines(self) -> None:
        report = _report()

        self.assertIn("J.A.R.V.I.S. FTAW Human Approval Review Decision Recorder Report", report)
        self.assertIn("decision status: blocked_gate_not_ready", report)
        self.assertIn("registry_update_dry_run_ready: false", report)
        self.assertIn("human approval review decision is not asset approval.", report)
        self.assertIn("human approval review decision is not registry mutation.", report)
        self.assertIn("human approval review decision is not verified evidence promotion.", report)
        self.assertIn("human approval review decision is not allocation advice.", report)
        self.assertIn("human approval review decision is not a buy/sell request.", report)
        self.assertIn("human approval review decision is not trade execution.", report)
    ...
    def test_synthetic_complete_report_records_registry_update_dry_run_decision(self) -> None:
        report = _report(
        ...
        self.assertIn("decision status: decision_recorded_for_registry_update_dry_run", report)
        self.assertIn("registry_update_dry_run_ready: true", report)
        self.assertIn("approved asset: false", report)
    ...
    def test_cli_report_runs_without_error(self) -> None:
        result = subprocess.run(
            [
                sys.executable,
                "-m",
                "jarvis.ftaw_human_approval_review_decision_recorder_report",
                SOURCE_REGISTRY,
                "None",
                URL_FETCH_CONFIG,
                INTAKE_CONFIG,
        ...
        self.assertIn("decision status: decision_recorded_for_registry_update_dry_run", result.stdout)
