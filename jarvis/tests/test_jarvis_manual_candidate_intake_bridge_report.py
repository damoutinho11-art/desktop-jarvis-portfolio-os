...
from jarvis.jarvis_manual_candidate_intake_bridge_report import build_report_from_path


SYNTHETIC_MULTI = "jarvis/data/jarvis_manual_candidate_intake_bridge.synthetic_multi.example.json"


class ManualCandidateIntakeBridgeReportTests(unittest.TestCase):
    def test_report_includes_route_chain_no_write_and_safety(self) -> None:
        report = build_report_from_path(SYNTHETIC_MULTI)

        self.assertIn("v4.50 manual watchlist entry -> v4.51 manual candidate intake bridge -> v4.49 candidate intake -> v4.27-v4.47 Phase 1 real evidence pipeline", report)
    ...
    def test_cli_default_and_input_work(self) -> None:
        for command in (
            [sys.executable, "-m", "jarvis.jarvis_manual_candidate_intake_bridge_report"],
            [sys.executable, "-m", "jarvis.jarvis_manual_candidate_intake_bridge_report", "--input", SYNTHETIC_MULTI],
        ):
            with self.subTest(command=command):
                result = subprocess.run(command, capture_output=True, text=True, check=True)
                self.assertIn("overall status:", result.stdout)
                self.assertIn("preview candidate count:", result.stdout)

    def test_module_import_is_safe_and_report_writes_no_files(self) -> None:
        module = importlib.import_module("jarvis.jarvis_manual_candidate_intake_bridge_report")
        ...
        self.assertIn("Manual Candidate Intake Bridge", report)
