...
from jarvis.ftaw_real_public_source_reference_intake_plan_report import (
...
def _report(
    ...
    return build_ftaw_real_public_source_reference_intake_plan_report(
        SOURCE_REGISTRY,
        None,
        URL_FETCH_CONFIG,
        INTAKE_CONFIG,
...
class FTAWRealPublicSourceReferenceIntakePlanReportTests(unittest.TestCase):
    def test_default_report_prints_blocked_status_and_safety_lines(self) -> None:
        report = _report()

        self.assertIn("public source reference plan status: BLOCKED_NOT_READY_FOR_PUBLIC_SOURCE_REFERENCES", report)
        self.assertIn("public source reference plan is not evidence collection.", report)
    ...
    def test_partial_report_prints_partial_status_and_blockers(self) -> None:
        ...
        self.assertIn("public source reference plan status: PARTIAL_PUBLIC_SOURCE_REFERENCE_PLAN_READY", report)
        self.assertIn("blocked reasons:", report)

    def test_synthetic_complete_report_prints_all_evidence_types_and_download_warning(self) -> None:
        report = _report(
            COMPLETE_QUEUE_CONFIG,
            COMPLETE_DECISION_CONFIG,
            COMPLETE_PREVIEW_CONFIG,
        ...
        self.assertIn("public source reference plan status: PUBLIC_SOURCE_REFERENCE_PLAN_READY", report)
        self.assertIn("public source reference plan does not fetch or download sources automatically.", report)
        self.assertIn("no automatic downloads: true", report)
    ...
    def test_cli_report_runs_without_error_and_prints_safety_lines(self) -> None:
        result = subprocess.run(
            [
                sys.executable,
                "-m",
                "jarvis.ftaw_real_public_source_reference_intake_plan_report",
                SOURCE_REGISTRY,
                "None",
                URL_FETCH_CONFIG,
                INTAKE_CONFIG,
        ...
        self.assertIn("public source reference plan status: PUBLIC_SOURCE_REFERENCE_PLAN_READY", result.stdout)
        self.assertIn("no automatic source fetching: true", result.stdout)
