...
from jarvis.ftaw_explicit_manual_apply_command_contract_report import build_ftaw_explicit_manual_apply_command_contract_report
...
def _report(
    ...
    return build_ftaw_explicit_manual_apply_command_contract_report(
        SOURCE_REGISTRY,
        None,
        URL_FETCH_CONFIG,
        INTAKE_CONFIG,
...
class FTAWExplicitManualApplyCommandContractReportTests(unittest.TestCase):
    def test_default_report_blocks_and_prints_safety_lines(self) -> None:
        report = _report()

        self.assertIn("J.A.R.V.I.S. FTAW Explicit Manual Apply Command Contract Report", report)
        self.assertIn("contract validation status: BLOCKED", report)
        self.assertIn("explicit manual apply command contract is not registry mutation.", report)
        self.assertIn("explicit manual apply command contract is not asset approval.", report)
        self.assertIn("explicit manual apply command contract is not verified evidence promotion.", report)
        self.assertIn("explicit manual apply command contract is not allocation advice.", report)
        self.assertIn("explicit manual apply command contract is not a buy/sell request.", report)
        self.assertIn("explicit manual apply command contract is not trade execution.", report)
    ...
    def test_synthetic_complete_report_validates_command(self) -> None:
        report = _report(
        ...
        self.assertIn("contract validation status: READY_FOR_MANUAL_REGISTRY_APPLY_EXECUTION_REVIEW", report)
        self.assertIn("asset_id match: true", report)
        self.assertIn("safety confirmations complete: true", report)
        self.assertIn("apply_executed: false", report)
        self.assertIn("registry_file_written: false", report)
        self.assertIn("approved_asset: false", report)
    ...
    def test_cli_report_runs_without_error(self) -> None:
        result = subprocess.run(
            [
                sys.executable,
                "-m",
                "jarvis.ftaw_explicit_manual_apply_command_contract_report",
                SOURCE_REGISTRY,
                "None",
                URL_FETCH_CONFIG,
                INTAKE_CONFIG,
        ...
        self.assertIn("contract validation status: READY_FOR_MANUAL_REGISTRY_APPLY_EXECUTION_REVIEW", result.stdout)
