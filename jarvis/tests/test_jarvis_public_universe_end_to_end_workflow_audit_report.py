...
from jarvis.jarvis_public_universe_end_to_end_workflow_audit_report import build_report_from_path, main
...
class PublicUniverseEndToEndWorkflowAuditReportTests(unittest.TestCase):
    def test_reports_include_expected_statuses(self) -> None:
        cases = (
            (DEFAULT_CONFIG, "PUBLIC_UNIVERSE_E2E_WORKFLOW_AUDIT_PARTIAL_SAFE"),
            (COMPLETE_CONFIG, "PUBLIC_UNIVERSE_E2E_WORKFLOW_AUDIT_READY_FOR_V5_FINAL_AUDIT_SAFE"),
            (PROBLEMATIC_CONFIG, "PUBLIC_UNIVERSE_E2E_WORKFLOW_AUDIT_PARTIAL_SAFE"),
            (AUTHORIZED_CONFIG, "PUBLIC_UNIVERSE_E2E_WORKFLOW_AUDIT_READY_TO_WRITE_SAFE"),
        )
    ...
    def test_report_includes_required_stage_ids_and_safety_statements(self) -> None:
        report = build_report_from_path(COMPLETE_CONFIG)
    ...
    def test_cli_default_and_input_work(self) -> None:
        ...
        with patch("sys.argv", ["prog"]), redirect_stdout(output):
            main()
        self.assertIn("PUBLIC_UNIVERSE_E2E_WORKFLOW_AUDIT_PARTIAL_SAFE", output.getvalue())

        output = io.StringIO()
        with patch("sys.argv", ["prog", "--input", COMPLETE_CONFIG]), redirect_stdout(output):
            main()
        report = output.getvalue()
        self.assertIn("stage id | version | status", report)
        self.assertIn("handoff | from | to", report)
        self.assertIn("V5_FINAL_AUDIT_READY_TO_RUN", report)

    def test_report_writes_no_files(self) -> None:
        report = build_report_from_path(AUTHORIZED_CONFIG)

        self.assertIn("PUBLIC_UNIVERSE_E2E_WORKFLOW_AUDIT_READY_TO_WRITE_SAFE", report)
