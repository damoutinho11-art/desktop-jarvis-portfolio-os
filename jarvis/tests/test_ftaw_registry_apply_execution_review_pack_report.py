...
from jarvis.ftaw_registry_apply_execution_review_pack_report import build_ftaw_registry_apply_execution_review_pack_report
...
def _report(
    ...
    return build_ftaw_registry_apply_execution_review_pack_report(
        SOURCE_REGISTRY,
        None,
        URL_FETCH_CONFIG,
        INTAKE_CONFIG,
...
class FTAWRegistryApplyExecutionReviewPackReportTests(unittest.TestCase):
    def test_default_report_blocks_and_prints_safety_lines(self) -> None:
        report = _report()

        self.assertIn("J.A.R.V.I.S. FTAW Registry Apply Execution Review Pack Report", report)
        self.assertIn("execution review status: BLOCKED", report)
        self.assertIn("execution review pack is not registry mutation.", report)
        self.assertIn("execution review pack is not asset approval.", report)
        self.assertIn("execution review pack is not verified evidence promotion.", report)
        self.assertIn("execution review pack is not allocation advice.", report)
        self.assertIn("execution review pack is not a buy/sell request.", report)
        self.assertIn("execution review pack is not trade execution.", report)
    ...
    def test_synthetic_complete_report_reaches_final_preflight(self) -> None:
        report = _report(
        ...
        self.assertIn("execution review status: READY_FOR_FINAL_MANUAL_REGISTRY_APPLY_PREFLIGHT", report)
    ...
    def test_cli_report_runs_without_error(self) -> None:
        result = subprocess.run(
            [
                sys.executable,
                "-m",
                "jarvis.ftaw_registry_apply_execution_review_pack_report",
                SOURCE_REGISTRY,
                "None",
                URL_FETCH_CONFIG,
                INTAKE_CONFIG,
        ...
        self.assertIn("execution review status: READY_FOR_FINAL_MANUAL_REGISTRY_APPLY_PREFLIGHT", result.stdout)
