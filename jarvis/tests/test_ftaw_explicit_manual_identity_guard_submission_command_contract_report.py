...
from jarvis.ftaw_explicit_manual_identity_guard_submission_command_contract_report import (
...
def _report(
    ...
    return build_ftaw_explicit_manual_identity_guard_submission_command_contract_report(
        SOURCE_REGISTRY,
        None,
        URL_FETCH_CONFIG,
        INTAKE_CONFIG,
...
class FTAWExplicitManualIdentityGuardSubmissionCommandContractReportTests(unittest.TestCase):
    def test_default_report_prints_blocked_status_checklist_and_safety_lines(self) -> None:
        report = _report()

        self.assertIn("contract status: BLOCKED_NO_IDENTITY_GUARD_SUBMISSION_REVIEW_GATE", report)
    ...
    def test_partial_report_prints_partial_status_missing_confirmations_and_blockers(self) -> None:
        report = _report(
            PARTIAL_QUEUE_CONFIG,
            PARTIAL_DECISION_CONFIG,
            PARTIAL_PREVIEW_CONFIG,
        ...
        self.assertIn("contract status: PARTIAL_MANUAL_IDENTITY_GUARD_SUBMISSION_COMMAND_RECORDED", report)
        self.assertIn("missing confirmation count:", report)
        self.assertIn("blocked reasons:", report)
    ...
    def test_complete_report_prints_ready_status_warnings_and_all_safety_lines(self) -> None:
        ...
        self.assertIn("contract status: READY_FOR_MANUAL_IDENTITY_GUARD_SUBMISSION_EXECUTION_REVIEW", report)
        self.assertIn("confirmation checklist:", report)
        for line in (
    ...
    def test_cli_report_runs_without_error_and_prints_confirmation_checklist_and_safety_lines(self) -> None:
        result = subprocess.run(
            [
                sys.executable,
                "-m",
                "jarvis.ftaw_explicit_manual_identity_guard_submission_command_contract_report",
                SOURCE_REGISTRY,
                "None",
                URL_FETCH_CONFIG,
                INTAKE_CONFIG,
        ...
        self.assertIn("contract status: READY_FOR_MANUAL_IDENTITY_GUARD_SUBMISSION_EXECUTION_REVIEW", result.stdout)
        self.assertIn("confirmation checklist:", result.stdout)
