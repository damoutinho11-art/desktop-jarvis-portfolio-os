...
from jarvis.ftaw_registry_update_dry_run_pack_report import build_ftaw_registry_update_dry_run_pack_report
...
def _report(
    ...
    return build_ftaw_registry_update_dry_run_pack_report(
        SOURCE_REGISTRY,
        None,
        URL_FETCH_CONFIG,
        INTAKE_CONFIG,
...
class FTAWRegistryUpdateDryRunPackReportTests(unittest.TestCase):
    def test_default_report_blocks_and_prints_safety_lines(self) -> None:
        report = _report()

        self.assertIn("J.A.R.V.I.S. FTAW Registry Update Dry-Run Pack Report", report)
        self.assertIn("registry update dry-run status: BLOCKED", report)
        self.assertIn("dry-run plan created: false", report)
        self.assertIn("registry update dry-run is not registry mutation.", report)
        self.assertIn("registry update dry-run is not asset approval.", report)
        self.assertIn("registry update dry-run is not verified evidence promotion.", report)
        self.assertIn("registry update dry-run is not allocation advice.", report)
        self.assertIn("registry update dry-run is not a buy/sell request.", report)
        self.assertIn("registry update dry-run is not trade execution.", report)
    ...
    def test_synthetic_complete_report_creates_dry_run_plan(self) -> None:
        report = _report(
        ...
        self.assertIn("registry update dry-run status: DRY_RUN_PLANNED", report)
        self.assertIn("dry-run plan created: true", report)
    ...
    def test_cli_report_runs_without_error(self) -> None:
        result = subprocess.run(
            [
                sys.executable,
                "-m",
                "jarvis.ftaw_registry_update_dry_run_pack_report",
                SOURCE_REGISTRY,
                "None",
                URL_FETCH_CONFIG,
                INTAKE_CONFIG,
        ...
        self.assertIn("registry update dry-run status: DRY_RUN_PLANNED", result.stdout)
