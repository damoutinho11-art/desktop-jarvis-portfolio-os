...
from jarvis.ftaw_real_manual_source_fact_identity_guard_bridge_report import (
...
def _report(
    ...
    return build_ftaw_real_manual_source_fact_identity_guard_bridge_report(
        SOURCE_REGISTRY,
        None,
        URL_FETCH_CONFIG,
        INTAKE_CONFIG,
...
class FTAWRealManualSourceFactIdentityGuardBridgeReportTests(unittest.TestCase):
    def test_default_report_prints_blocked_status_and_safety_lines(self) -> None:
        report = _report()

        self.assertIn("bridge status: BLOCKED_NO_MANUAL_SOURCE_FACTS", report)
        self.assertIn("identity field coverage table:", report)
    ...
    def test_partial_report_prints_partial_status_and_blockers(self) -> None:
        ...
        self.assertIn("bridge status: PARTIAL_IDENTITY_GUARD_BRIDGE_READY", report)
        self.assertIn("blocked reasons:", report)

    def test_synthetic_complete_report_prints_identity_coverage_consistency_and_warnings(self) -> None:
        report = _report(
            COMPLETE_QUEUE_CONFIG,
            COMPLETE_DECISION_CONFIG,
            COMPLETE_PREVIEW_CONFIG,
        ...
        self.assertIn("bridge status: READY_FOR_MANUAL_IDENTITY_GUARD_REVIEW", report)
        self.assertIn("identity field coverage table:", report)
        self.assertIn("identity consistency checks:", report)
        self.assertIn("identity guard bridge is not identity confirmation.", report)
        self.assertIn("identity guard bridge is not evidence verification.", report)
    ...
    def test_cli_report_runs_without_error_and_prints_safety_lines(self) -> None:
        result = subprocess.run(
            [
                sys.executable,
                "-m",
                "jarvis.ftaw_real_manual_source_fact_identity_guard_bridge_report",
                SOURCE_REGISTRY,
                "None",
                URL_FETCH_CONFIG,
                INTAKE_CONFIG,
        ...
        self.assertIn("bridge status: READY_FOR_MANUAL_IDENTITY_GUARD_REVIEW", result.stdout)
