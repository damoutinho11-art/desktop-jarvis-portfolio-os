...
from jarvis.ftaw_identity_guard_submission_execution_review_pack_report import (
...
def _report(
    ...
    return build_ftaw_identity_guard_submission_execution_review_pack_report(
        SOURCE_REGISTRY,
        None,
        URL_FETCH_CONFIG,
        INTAKE_CONFIG,
...
class FTAWIdentityGuardSubmissionExecutionReviewPackReportTests(unittest.TestCase):
    def test_default_report_prints_blocked_status_final_preflight_and_safety_lines(self) -> None:
        report = _report()

        self.assertIn("execution review status: BLOCKED_NO_IDENTITY_GUARD_SUBMISSION_COMMAND_CONTRACT", report)
        self.assertIn("final preflight ready: false", report)
    ...
    def test_partial_report_prints_partial_status_and_blockers(self) -> None:
        ...
        self.assertIn("execution review status: PARTIAL_IDENTITY_GUARD_SUBMISSION_EXECUTION_REVIEW_READY", report)
        self.assertIn("blocked reasons:", report)

    def test_complete_report_prints_final_preflight_warnings_and_safety_lines(self) -> None:
        report = _report(
            COMPLETE_QUEUE_CONFIG,
            COMPLETE_DECISION_CONFIG,
            COMPLETE_PREVIEW_CONFIG,
        ...
        self.assertIn("execution review status: READY_FOR_FINAL_MANUAL_IDENTITY_GUARD_SUBMISSION_PREFLIGHT", report)
        self.assertIn("final preflight ready: true", report)
        for line in (
    ...
    def test_cli_report_runs_without_error_and_prints_final_preflight_and_safety_lines(self) -> None:
        result = subprocess.run(
            [
                sys.executable,
                "-m",
                "jarvis.ftaw_identity_guard_submission_execution_review_pack_report",
                SOURCE_REGISTRY,
                "None",
                URL_FETCH_CONFIG,
                INTAKE_CONFIG,
        ...
        self.assertIn("execution review status: READY_FOR_FINAL_MANUAL_IDENTITY_GUARD_SUBMISSION_PREFLIGHT", result.stdout)
        self.assertIn("final preflight ready: true", result.stdout)
