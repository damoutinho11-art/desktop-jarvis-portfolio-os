...
from jarvis.ftaw_source_collection_report import build_ftaw_source_collection_report
...
class FTAWSourceCollectionReportTests(unittest.TestCase):
    def test_report_contains_summary_and_safety_lines(self) -> None:
        report = build_ftaw_source_collection_report(
            SOURCE_REGISTRY,
            REVIEWED_REGISTRY,
            QUEUE_CONFIG,
            BATCH_CONFIG,
        ...
        self.assertIn("FTAW collection pack status: READY", report)
        self.assertIn("target asset: ftaw_global_core_candidate", report)
        self.assertIn("total collection items: 7", report)
        self.assertIn("sample intake template:", report)
    ...
    def test_cli_runs_without_error(self) -> None:
        result = subprocess.run(
            [
                sys.executable,
                "-m",
                "jarvis.ftaw_source_collection_report",
                SOURCE_REGISTRY,
                REVIEWED_REGISTRY,
                QUEUE_CONFIG,
                BATCH_CONFIG,
        ...
        self.assertIn("J.A.R.V.I.S. FTAW Source Collection Pack Report", result.stdout)
        self.assertIn("total collection items: 7", result.stdout)
