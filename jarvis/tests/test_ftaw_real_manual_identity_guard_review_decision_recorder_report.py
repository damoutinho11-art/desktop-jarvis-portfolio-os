...
from jarvis.ftaw_real_manual_identity_guard_review_decision_recorder_report import (
...
def _report(
    ...
    return build_ftaw_real_manual_identity_guard_review_decision_recorder_report(
        SOURCE_REGISTRY,
        None,
        URL_FETCH_CONFIG,
        INTAKE_CONFIG,
...
class FTAWManualIdentityGuardReviewDecisionRecorderReportTests(unittest.TestCase):
    def test_default_report_prints_blocked_status_decision_table_and_safety_lines(self) -> None:
        report = _report()

        self.assertIn("recorder status: BLOCKED_NO_IDENTITY_GUARD_REVIEW_PACKET", report)
        self.assertIn("decision table:", report)
        self.assertIn("no identity guard pass created: true", report)

    def test_partial_report_prints_blockers(self) -> None:
        report = _report(
            PARTIAL_QUEUE_CONFIG,
            PARTIAL_DECISION_CONFIG,
            PARTIAL_PREVIEW_CONFIG,
    ...
    def test_complete_report_prints_decision_table_safety_and_warnings(self) -> None:
        report = _report(
            COMPLETE_QUEUE_CONFIG,
            COMPLETE_DECISION_CONFIG,
            COMPLETE_PREVIEW_CONFIG,
        ...
        self.assertIn("recorder status: MANUAL_IDENTITY_GUARD_DECISIONS_RECORDED_FOR_DRY_RUN_SUBMISSION_REVIEW", report)
        self.assertIn("decision table:", report)
        self.assertIn("decision recording is not identity confirmation.", report)
        self.assertIn("decision recording is not evidence verification.", report)
        self.assertIn("decision recording is not asset approval.", report)
        self.assertIn("decision recording is not a buy signal.", report)
    ...
    def test_cli_report_runs_without_error_and_prints_safety_lines(self) -> None:
        result = subprocess.run(
            [
                sys.executable,
                "-m",
                "jarvis.ftaw_real_manual_identity_guard_review_decision_recorder_report",
                SOURCE_REGISTRY,
                "None",
                URL_FETCH_CONFIG,
                INTAKE_CONFIG,
        ...
        self.assertIn("recorder status: MANUAL_IDENTITY_GUARD_DECISIONS_RECORDED_FOR_DRY_RUN_SUBMISSION_REVIEW", result.stdout)
        self.assertIn("no queue eligibility created: true", result.stdout)
