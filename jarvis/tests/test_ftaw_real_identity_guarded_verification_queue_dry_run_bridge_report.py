...
from jarvis.ftaw_real_identity_guarded_verification_queue_dry_run_bridge_report import (
...
def _report(
    ...
    return build_ftaw_real_identity_guarded_verification_queue_dry_run_bridge_report(
        SOURCE_REGISTRY,
        None,
        URL_FETCH_CONFIG,
        INTAKE_CONFIG,
...
class FTAWRealIdentityGuardedVerificationQueueDryRunBridgeReportTests(unittest.TestCase):
    def test_default_report_prints_blocked_status_preview_table_and_safety_lines(self) -> None:
        report = _report()

        self.assertIn("queue dry-run bridge status: BLOCKED_NO_MANUAL_IDENTITY_GUARD_RESULTS", report)
        self.assertIn("preview item table:", report)
        self.assertIn("blocked reasons:", report)
        self.assertIn("no real queue item created: true", report)
    ...
    def test_partial_report_prints_partial_status_and_blockers(self) -> None:
        ...
        self.assertIn("queue dry-run bridge status: PARTIAL_VERIFICATION_QUEUE_DRY_RUN_READY", report)
        self.assertIn("dry-run preview item count: 1", report)
        self.assertIn("blocked reasons:", report)
    ...
    def test_complete_report_prints_ready_status_warnings_and_safety_lines(self) -> None:
        ...
        self.assertIn("queue dry-run bridge status: VERIFICATION_QUEUE_DRY_RUN_READY_FOR_MANUAL_REVIEW", report)
        self.assertIn("preview item table:", report)
        for line in (
    ...
    def test_cli_report_runs_without_error_and_prints_preview_table_and_safety_lines(self) -> None:
        result = subprocess.run(
            [
                sys.executable,
                "-m",
                "jarvis.ftaw_real_identity_guarded_verification_queue_dry_run_bridge_report",
                SOURCE_REGISTRY,
                "None",
                URL_FETCH_CONFIG,
                INTAKE_CONFIG,
        ...
        self.assertIn("queue dry-run bridge status: VERIFICATION_QUEUE_DRY_RUN_READY_FOR_MANUAL_REVIEW", result.stdout)
        self.assertIn("preview item table:", result.stdout)
