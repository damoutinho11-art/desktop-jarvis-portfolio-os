"""Readable report for FTAW real verified evidence promotion dry-run pack."""
...
def build_ftaw_real_verified_evidence_promotion_dry_run_pack_report(
    ...
    pack = build_ftaw_real_verified_evidence_promotion_dry_run_pack_from_files(
    ...
    lines = [
        "J.A.R.V.I.S. FTAW Real Verified Evidence Promotion Dry-Run Pack Report",
        "Automated structure. Manual trust.",
        f"target asset: {pack.target_asset}",
        f"promotion dry-run status: {pack.promotion_dry_run_status}",
        f"upstream v4.41 status: {pack.upstream_v4_41_status}",
        f"preview record count: {pack.preview_record_count}",
        f"planned promotion item count: {pack.planned_promotion_item_count}",
        f"missing item count: {pack.missing_item_count}",
        f"manual/private outstanding count: {pack.manual_private_outstanding_count}",
        "planned item table:",
        "evidence_type | source_reference_id | preview_status | planned_promotion_status | promotion_dry_run | verified_evidence_preview | evidence_verified | verified_evidence_promoted | approved_asset | registry_mutation | allocation_recommendation | buy_signal | trade_executed",
    ]
    if pack.planned_items:
        for item in pack.planned_items:
            lines.append(
                f"{item.evidence_type} | {item.source_reference_id} | {item.preview_status} | "
                f"{item.planned_promotion_status} | {str(item.promotion_dry_run).lower()} | "
                f"{str(item.verified_evidence_preview).lower()} | {str(item.evidence_verified).lower()} | "
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Build FTAW real verified evidence promotion dry-run pack report.")
