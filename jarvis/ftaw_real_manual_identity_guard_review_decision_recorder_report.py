"""Readable report for FTAW manual identity guard review decision recorder."""
...
def build_ftaw_real_manual_identity_guard_review_decision_recorder_report(
    ...
    pack = build_ftaw_real_manual_identity_guard_review_decision_recorder_from_files(
    ...
    lines = [
        "J.A.R.V.I.S. FTAW Real Manual Identity Guard Review Decision Recorder Report",
        "Automated structure. Manual trust.",
        f"target asset: {pack.target_asset}",
        f"recorder status: {pack.recorder_status}",
        f"upstream v4.32 bridge status: {pack.upstream_bridge_status}",
        f"decisions provided count: {pack.decisions_provided_count}",
        ...
        "decision table:",
        "evidence_type | source_reference_id | reviewer_decision | decision_status | reviewed_by_user | evidence_verified | identity_guard_pass_created | queue_eligibility_created | reason",
    ]
    if pack.decision_records:
        for record in pack.decision_records:
            lines.append(
                f"{record.evidence_type} | {record.source_reference_id} | {record.reviewer_decision} | "
                f"{record.decision_status} | {str(record.reviewed_by_user).lower()} | "
                f"{str(record.evidence_verified).lower()} | {str(record.identity_guard_pass_created).lower()} | "
                f"{str(record.queue_eligibility_created).lower()} | {record.reason}"
            )
    else:
        lines.append("none")
    lines.append("rejected entry table:")
    lines.append("evidence_type | source_reference_id | reason")
    if pack.rejected_entries:
        ...
        lines.append("none")
    lines.append("manual/private outstanding table:")
    lines.append("evidence_type | reason | next manual action")
    ...
    lines.extend(
        [
            ...
            "decision recording is not identity confirmation.",
            "decision recording is not evidence verification.",
            "decision recording is not queue eligibility.",
            "decision recording is not asset approval.",
            "decision recording is not registry mutation.",
            "decision recording is not allocation advice.",
            "decision recording is not buy/sell/trade execution.",
            "decision recording is not a buy signal.",
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Build FTAW manual identity guard review decision recorder report.")
    ...
    print(
        build_ftaw_real_manual_identity_guard_review_decision_recorder_report(
