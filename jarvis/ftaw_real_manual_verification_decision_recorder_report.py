"""Readable report for FTAW real manual verification decision recorder."""

from __future__ import annotations

from pathlib import Path

from .ftaw_real_manual_verification_decision_recorder import build_ftaw_real_manual_verification_decision_recorder_from_files


def build_ftaw_real_manual_verification_decision_recorder_report(
    ...
    pack = build_ftaw_real_manual_verification_decision_recorder_from_files(
    ...
    lines = [
        "J.A.R.V.I.S. FTAW Real Manual Verification Decision Recorder Report",
        "Automated structure. Manual trust.",
        f"target asset: {pack.target_asset}",
        f"decision recorder status: {pack.decision_recorder_status}",
        f"upstream v4.39 status: {pack.upstream_v4_39_status}",
        f"manual decision count: {pack.manual_decision_count}",
        f"accepted for preview count: {pack.accepted_for_preview_count}",
        f"rejected/needs_correction count: {pack.rejected_needs_correction_count}",
        f"missing decision count: {pack.missing_decision_count}",
        ...
        "decision table:",
        "evidence_type | source_reference_id | manual_verification_decision | decision_status | decision_recorded | verified_evidence_preview_ready | evidence_verified | verified_evidence_promoted | queue_eligibility_created | approved_asset | registry_mutation | buy_signal | reviewer_notes",
    ]
    if pack.decision_records:
        for record in pack.decision_records:
            lines.append(
                f"{record.evidence_type} | {record.source_reference_id} | {record.manual_verification_decision} | "
                f"{record.decision_status} | {str(record.decision_recorded).lower()} | "
                f"{str(record.verified_evidence_preview_ready).lower()} | {str(record.evidence_verified).lower()} | "
                f"{str(record.verified_evidence_promoted).lower()} | {str(record.queue_eligibility_created).lower()} | "
                f"{str(record.approved_asset).lower()} | {str(record.registry_mutation).lower()} | "
                f"{str(record.buy_signal).lower()} | {record.reviewer_notes}"
            )
    else:
        lines.append("none")
    ...
    lines.extend(
        [
            ...
            "manual verification decision recording is not evidence verification.",
            "manual verification decision recording is not verified evidence promotion.",
            "manual verification decision recording is not queue eligibility.",
            "manual verification decision recording is not asset approval.",
            "manual verification decision recording is not registry mutation.",
            "manual verification decision recording is not allocation advice.",
            "manual verification decision recording is not a buy/sell request.",
            "manual verification decision recording is not trade execution.",
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Build FTAW real manual verification decision recorder report.")
    ...
    print(
        build_ftaw_real_manual_verification_decision_recorder_report(
