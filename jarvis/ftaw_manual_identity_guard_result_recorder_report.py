"""Readable report for FTAW manual identity guard result recorder."""
...
def build_ftaw_manual_identity_guard_result_recorder_report(
    ...
    pack = build_ftaw_manual_identity_guard_result_recorder_from_files(
    ...
    lines = [
        "J.A.R.V.I.S. FTAW Manual Identity Guard Result Recorder Report",
        "Automated structure. Manual trust.",
        f"target asset: {pack.target_asset}",
        f"result recorder status: {pack.result_recorder_status}",
        f"upstream v4.37 status: {pack.upstream_v4_37_status}",
        f"manual result count: {pack.manual_result_count}",
        f"pass count: {pack.pass_count}",
        f"fail/needs_correction count: {pack.fail_needs_correction_count}",
        f"missing result count: {pack.missing_result_count}",
        f"manual/private outstanding count: {pack.manual_private_outstanding_count}",
        "result table:",
        "evidence_type | source_reference_id | manual_identity_guard_result | result_status | manual_result_recorded | identity_guard_executed_by_system | evidence_verified | queue_eligibility_created | approved_asset | registry_mutation | buy_signal | reviewer_notes",
    ]
    if pack.result_records:
        for record in pack.result_records:
            lines.append(
                f"{record.evidence_type} | {record.source_reference_id} | {record.manual_identity_guard_result} | "
                f"{record.result_status} | {str(record.manual_result_recorded).lower()} | "
                f"{str(record.identity_guard_executed_by_system).lower()} | {str(record.evidence_verified).lower()} | "
                f"{str(record.queue_eligibility_created).lower()} | {str(record.approved_asset).lower()} | "
                f"{str(record.registry_mutation).lower()} | {str(record.buy_signal).lower()} | {record.reviewer_notes}"
            )
    else:
        lines.append("none")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Build FTAW manual identity guard result recorder report.")
    ...
    parser.add_argument(
        "manual_identity_guard_result_recorder_config_path",
        nargs="?",
        default="jarvis/data/ftaw_manual_identity_guard_result_recorder.example.json",
    )
    args = parser.parse_args()
    print(
        build_ftaw_manual_identity_guard_result_recorder_report(
