"""Readable report for FTAW real manual source fact identity guard bridge."""
...
def build_ftaw_real_manual_source_fact_identity_guard_bridge_report(
    ...
    pack = build_ftaw_real_manual_source_fact_identity_guard_bridge_from_files(
    ...
    lines = [
        "J.A.R.V.I.S. FTAW Real Manual Source Fact Identity Guard Bridge Report",
        "Automated structure. Manual trust.",
        f"target asset: {pack.target_asset}",
        f"bridge status: {pack.bridge_status}",
        f"upstream status: {pack.upstream_manual_source_fact_status}",
        (
            "public fact coverage summary: "
            f"{pack.present_public_fact_type_count} / {pack.required_public_fact_type_count} present; "
            f"{pack.missing_public_fact_type_count} missing"
        ),
        "present public fact types: " + (", ".join(pack.present_public_fact_types) or "none"),
        "missing public fact types: " + (", ".join(pack.missing_public_fact_types) or "none"),
        "identity field coverage table:",
        "field_name | present | source_evidence_type | value_preview",
    ]
    for item in pack.identity_field_coverage:
        lines.append(f"{item.field_name} | {str(item.present).lower()} | {item.source_evidence_type} | {item.value_preview}")
    lines.extend(["identity consistency checks:", "check_name | status | reason"])
    for check in pack.identity_consistency_checks:
        lines.append(f"{check.check_name} | {check.status} | {check.reason}")
    lines.extend(["manual/private outstanding items:", "evidence_type | reason | next manual action"])
    ...
    preview = pack.identity_review_packet_preview
    lines.extend(
        [
            f"next manual action: {pack.next_manual_action}",
            f"review_packet_created: {str(preview.review_packet_created).lower()}",
            f"review_packet_preview_only: {str(preview.review_packet_preview_only).lower()}",
            f"identity_guard_pass_created: {str(preview.identity_guard_pass_created).lower()}",
            f"queue_eligibility_created: {str(preview.queue_eligibility_created).lower()}",
            f"evidence_verified: {str(preview.evidence_verified).lower()}",
            f"approved_asset: {str(preview.approved_asset).lower()}",
            f"buy_signal: {str(preview.buy_signal).lower()}",
            "identity guard bridge is not identity confirmation.",
            "identity guard bridge is not evidence verification.",
            "identity guard bridge is not queue eligibility.",
            "identity guard bridge is not asset approval.",
            "identity guard bridge is not registry mutation.",
            "identity guard bridge is not allocation advice.",
            "identity guard bridge is not a buy/sell request.",
            "identity guard bridge is not trade execution.",
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Build FTAW manual source fact identity guard bridge report.")
    ...
    print(
        build_ftaw_real_manual_source_fact_identity_guard_bridge_report(
